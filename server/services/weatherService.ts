...
// Maps WMO code → OpenWeatherMap icon code so existing <img> tags still work
...
async function fetchCurrentByCoords(
  ...
  const icon = wmoToIcon(c.weather_code, Boolean(c.is_day));
...
async function fetchForecastByCoords(lat: number, lon: number, cityName: string, country: string) {
  ...
  const list = daily.time.map((dateStr, i) => {
    const condition = wmoToCondition(daily.weather_code[i]);
    const icon = wmoToIcon(daily.weather_code[i], true);
